...
class Perceptron:
    ...
    def kernelFunction_polynomial(self,x,z):
        self.a = 1                                 # a >= 0
        self.b = 1                                 # b > 0
    ...
    def trainningMachine(self,x):                       #接受数据集，x 为二维数组，带labels
        _weight = [0] * len(x[0][1:])
        _dot_product = 0.0
        _b = 1
        _w_trained = []
    ...
    def kernelTraining(self,x):
        _a = [0] * len(x[0][1:])
        for _loop in range(self.lr_loop):
            for _i in range(len(x)):
                _sum = 0.0
                _k = self.kernelFunction_polynomial(x[_i][1:],x[_i][1:])
                _a = [x * _k for x in _a]
                _sum = sum(_a)
                _yPredict = self.sig_fun(_sum)
                if _yPredict != int(x[_i][0]):
                    _update = int(x[_i][0]) * self.lr_rate
                    _a = [x + _update for x in _a]
                else:
                    continue
        return _a
...
f = open(r'C:\Users\23619\Desktop\课程作业和课件\Graduate program\Spring 2021\Machine learning\Mini-project\test-binary2.txt','r')
ds = f.readlines()
list_labels = []
list_features = []
dict_features = {}
...
PEC = Perceptron()
list_weight = PEC.kernelTraining(list_new_features)
print(list_weight)
...
count_correct = 0
count_incorrect = 0
for i in list_new_features[1:50]:
